...
// dinh nghia cac trang thai cua tien trinh
typedef enum{
	READY,
	RUNNING,
	WAITING,
...
typedef struct {
	Node* head; // con tro den node dau tien
}LinkedList;


// functions: 
void initList(LinkedList*); // init linkedlist
void insertProcess(LinkedList*, int, const char*, Status, int);  // insert date 
Node* findReadyHighestPriority(LinkedList*);  // funcion
void removeAtIndex(LinkedList*, int); // function
void removeAllTerminated(LinkedList*); // function
void printList(LinkedList*); // function
Status convertToStatus(int); // function
...
int main()
{
    LinkedList list;	// khai bao 1 bien list co kieu LinkedList
    initList(&list);	// chuyen con tro toi bien vua duoc khai bao
    int choice;  		// bien luu choice
    ...
    do {
        ...
        if (scanf("%d", &choice) != 1) {
            ...
            continue;	// bo qua phan con lai trong vong lap
        }
        ...
        printf("\n");
    } while(choice != 0);	// choice == 0 thi thoat chuong trinh
    
    // Giai phong bo nho danh sach (neu can)
    Node* current = list.head;
    while(current != NULL) {	// current != NULL thi tiep tuc giai phong bo nho
        Node* temp = current;
...
Node* findReadyHighestPriority(LinkedList* list) {
    ...
    while (temp != NULL) {	// dieu khien dung
